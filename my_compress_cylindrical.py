...
parser.add_argument('--posQ', default=8, type=int, help='Quantization scale (mm).')

# ===== Cylindrical params (只影响坐标系，不改网络) =====
parser.add_argument('--theta_scale_mm', type=float, default=4096.0, help='a_mm = theta(rad) * theta_scale_mm (mm). Must match decoder & training.')
parser.add_argument('--shift_mm', type=float, default=131072.0, help='Shift before quantization (mm). Default matches original style.')
...
# warm up (坐标范围无所谓，主要是触发 torchsparse 的 kernel map)
...
with torch.no_grad():
    for file_idx in tqdm(range(len(file_path_ls))):
        ...
        # ==============================
        xyz = torch.tensor(xyz_ls[file_idx], dtype=torch.float32)  # meters (通常) 或 mm (pre-quantized)
        coords3 = cart_to_cyl_grid(xyz, cyl_cfg, is_pre_quantized=args.is_data_pre_quantized)
        N = int(coords3.shape[0])  # 用“占据 voxel 数”做 bpp 分母更合理
        ...
        base_x_coords = base_x_coords[:, 1:].cpu().numpy().astype(np.int32)  # (n, 3) in cyl-grid
        base_x_feats = base_x_feats.cpu().numpy().astype(np.uint8)           # (n, 1)

        with open(compressed_file_path, 'wb') as f:
            # header: posQ + theta_scale_mm
            f.write(np.array(args.posQ, dtype=np.float16).tobytes())
            f.write(np.array(args.theta_scale_mm, dtype=np.float16).tobytes())
