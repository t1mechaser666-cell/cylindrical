...
# ⚠️ 必须与 compress.py 使用同一个 Network 结构
...
# 如果你始终固定 shift_mm=131072，可以不暴露；这里保留参数方便你实验
parser.add_argument('--shift_mm', type=float, default=131072.0, help='Shift before quantization (mm). Must match encoder/training.')
...
with torch.no_grad():
    for file_path in tqdm(file_path_ls):
        ...
        with open(file_path, 'rb') as f:
            posQ = float(np.frombuffer(f.read(2), dtype=np.float16)[0])
            theta_scale_mm = float(np.frombuffer(f.read(2), dtype=np.float16)[0])
